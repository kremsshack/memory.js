...
function melangeSimple(tableau) {
  const tableauMelange = [].concat(tableau);
  tableauMelange.sort(function () {
...
function reinitialiserCartesRetournees() {
  cartesRetournees.value = [];
  verrouillePlateau.value = false;
}

function verifierPaire() {
  const carte1 = cartesRetournees.value[0];
  const carte2 = cartesRetournees.value[1];
...
function definirDifficulte(difficulte) {
  ...
  pairesSelectionnees.forEach(function (paire) {
  ...
  cartes.value = cartesMelangees.map(function (carte, index) {
    return {
      valeur: carte.valeur,
      image: carte.image,
...
const styleGrille = computed(function () {
  let cols = 2;
  if (
    difficulteActuelle.value === "intermediaire" ||
    difficulteActuelle.value === "difficile"
  ) {
    cols = 4;
  }
...
<template>
  <div class="conteneur-jeu">
    ...
    <div class="stats-jeu">
      <p>Coups : **{{ coups }}**</p>
      <button
        v-if="difficulteActuelle"
        @click="recommencerJeu"
        class="bouton-recommencer"
      >
        Recommencer
      </button>
    </div>
    ...
    <div v-if="cartes.length > 0" class="plateau-jeu" :style="styleGrille">
      <div
        ...
        class="carte"
        :class="{
          'est-trouvee': carte.estTrouvee,
          'est-retournee': carte.estRetournee || carte.estTrouvee,
        }"
    ...
    <div v-else>
      <p style="text-align: center; font-size: 1.2em">
        Veuillez choisir un niveau pour commencer la partie.
      </p>
    </div>

    <h2
      v-if="cartes.length > 0 && cartes.every((c) => c.estTrouvee)"
